...
class App extends Component {
  ...
  getPotentialBuyers() {
    ...
    }).catch(() => toast.error("Failed at getting Buyers"))
    // axios (GET)
  ...
  addCar() {
    ...
    axios.post('https://joes-autos-herokuapp.com/api/vehicles', newCar).then(results =>{
      toast.success("Successfully created new car.")
      this.setState({'vehiclesToDisplay': results.data})
    }).catch(() => toast.error("Failed to create vehicle."))
    // axios (POST)
    // setState with response -> vehiclesToDisplay
  }
  ...
  addBuyer() {
    ...
    axios.put('https://joes-autos-herokuapp.com/api/buyers', newBuyer).then(results =>{
  ...
  deleteBuyer(id) {
    // axios (DELETE)
    axious.delete(`https://joes-autos-herokuapp.com/api/buyers/${id}`).then(results => {
